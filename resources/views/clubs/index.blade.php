    ...
    <div class="max-w-7xl mx-auto space-y-8 p-2 sm:p-4">
        ...
        <div class="relative rounded-2xl overflow-hidden bg-gradient-to-r from-emerald-700 via-green-600 to-teal-600 p-6 sm:p-8 text-white shadow-xl">
            ...
            <div class="relative z-10">
                ...
                <p class="text-green-100 text-sm sm:text-base">Explore football clubs from around the world.</p>
            </div>
        </div>
        ...
        <form method="GET" class="rounded-2xl border border-zinc-200/80 dark:border-zinc-700/80 bg-white dark:bg-zinc-800 p-5 shadow-sm">
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex-1 min-w-[180px]">
                    <label class="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5">Club Name</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search clubs..." class="w-full p-4 rounded-xl border-zinc-200 dark:border-zinc-600 dark:bg-zinc-900/50 dark:text-white focus:border-green-500 focus:ring-green-500/20 text-sm">
                </div>
                <div class="w-40">
                    <label class="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5">Country</label>
                    <input type="text" name="country" value="{{ request('country') }}" placeholder="e.g. England" class="w-full p-4 rounded-xl border-zinc-200 dark:border-zinc-600 dark:bg-zinc-900/50 dark:text-white focus:border-green-500 focus:ring-green-500/20 text-sm">
                </div>
                ...
                <a href="{{ route('clubs.index') }}" class="rounded-xl border border-zinc-200 dark:border-zinc-600 px-5 py-2.5 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition">Clear</a>
                @endif
            </div>
        </form>
        ...
        <div class="grid gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            ...
            <a href="{{ route('clubs.show', $club->id) }}" class="group block rounded-2xl border border-zinc-200/80 dark:border-zinc-700/80 bg-white dark:bg-zinc-800 shadow-sm hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
                <div class="p-5">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-zinc-900 dark:text-white group-hover:text-green-600 transition text-sm truncate">{{ $club->name }}</div>
                            <div class="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">{{ $club->country }}</div>
                        </div>
                    </div>
                    ...
                    @endif
                    @if($club->founded)
                    <div class="flex items-center gap-1.5 text-xs text-zinc-400 dark:text-zinc-500 mt-1">
                        <span>📅</span>
